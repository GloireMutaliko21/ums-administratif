...
export const getAllocationsPerAgent = async (req, res, next) => {
    try {
        ...
        const allocation = await dbSequelize.query(`SELECT sum(nbEnfant) AS nbEnfant, sum(jours) AS jours, sum(nbEnfant * taux * jours) AS total FROM allocations WHERE (agentId = '${agentId}' AND createdAt LIKE '${mounth}%')`, { type: QueryTypes.SELECT });
