            ...
            <section className='flex items-start gap-8 2xl:gap-0 w-full justify-around 2xl:justify-evenly mt-8 p-5 shadow border-t'>
                <div className='flex flex-col'>
                    ...
                    <input
                        ...
                        value={agentToPay?.grade.taux.conge}
                        onChange={() => { }}
                        disabled
                    />
                    <Button
                        ...
                        onClick={() => {
                            handlePost(localUserData.token, headers, JSON.stringify({ jours, taux: agentToPay?.grade.taux.conge, agentId: agentToPay.id }), `${PAIE_BASE_URL}/conge/new`, setTotalMois, 'newConge', setInLoading, () => { }, `${PAIE_BASE_URL}/conge/${agentToPay.id}?mounth=2023-01`, () => { }, () => { });
                        }}
                    />
                </div>
                <div className='border-t py-2 px-4 shadow flex flex-col items-center gap-3 text-center'>
                    ...
                    <div className='min-h-min h-12 min-w-min w-12 flex items-center justify-center border-4 border-sky-500 text-2xl text-amber-500 font-extrabold rounded-full'>
                        <p> {isNaN(jours * agentToPay?.grade.taux.conge) ? '0' : jours * agentToPay?.grade.taux.conge}</p>
                    </div>
                </div>
                {totalMois &&
