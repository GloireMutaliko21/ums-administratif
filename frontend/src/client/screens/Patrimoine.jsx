...
import { patrimoineLinks } from '../../admin/data/componentsData';
    ...
    return (
        <div className='relative'>
            <nav className='fixed z-20 -mt-5 py-4 left-60 right-5 text-slate-500 text-sm bg-white p-2 border-b shadow-inherit flex items-center justify-between'>
                <h1 className='font-extrabold text-3xl text-slate-900'>Patrimoine</h1>
                <div>
                    {
                        patrimoineLinks.map(({ path, titre }) =>
                            <NavLink
                                key={path}
                                to={`/index/cpatrimoine/${path}`}
